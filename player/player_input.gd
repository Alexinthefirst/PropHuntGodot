...
@export var transforming := false
@export var return_normal := false
@export var change_role := false
@export var taking_damage := false
@export var currentProp := ""

@export var hunter_blindness := false
var HUD
...
@onready var _camera_raycast: RayCast3D = get_parent().get_node("PlayerModel/RayCast3D")
...
func _process(delta):
	...
	# Update our HUD locally
	HUD.set_life(get_parent().health)
	HUD.set_role(get_parent().player_role)
	
	if hunter_blindness:
		HUD.get_node("HunterBlindness").visible = true
	else:
		HUD.get_node("HunterBlindness").visible = false
	...
	if Input.is_action_just_pressed("blindness"):
		if get_parent().get_parent().get_parent().get_node("HUD").get_node("HunterBlindness").visible == true:
			get_parent().get_parent().get_parent().get_node("HUD").get_node("HunterBlindness").visible = false
		else:
			get_parent().get_parent().get_parent().get_node("HUD").get_node("HunterBlindness").visible = true
	
	if Input.is_action_just_pressed("start_game") && multiplayer.is_server():
		get_parent().get_parent().get_parent().start_game()
	
	if Input.is_action_just_pressed("change_role"):
		change_role = true
	
	if Input.is_action_just_pressed("respawn"):
		get_parent().respawn.rpc()
	...
	if Input.is_action_just_pressed("left_click"):
		# ----------PROP------------
		if get_parent().player_role == ROLES.PROP:
			if _camera_raycast.is_colliding():
				var _aim_collider = _camera_raycast.get_collider()
				if _aim_collider.is_in_group("props"):
					print("Transforming")
					print(_aim_collider.get_scene_file_path())
					print("=============")
					transforming = true
					currentProp = _aim_collider.get_scene_file_path().trim_prefix("res://props/").trim_suffix(".tscn")
		
		# ---------HUNTER-------------
		if get_parent().player_role == ROLES.HUNTER:
			if _camera_raycast.is_colliding():
				var _aim_collider = _camera_raycast.get_collider()
				print(_aim_collider)
				if _aim_collider.is_in_group("props"):
					print("Attacking prop!")
					taking_damage = true
				if _aim_collider.is_in_group("player"):
					_aim_collider.take_damage.rpc()
...
func _on_hunter_blindness_timer_timeout():
	hunter_blindness = false
